  ...
  return (
    <div className="glass-card glass-card-hover rounded-3xl overflow-hidden flex flex-col group border border-slate-200/90 dark:border-slate-800/90 bg-white/70 dark:bg-[#0B132B]/75 transition-all duration-300">
      ...
      <div className="relative aspect-[16/11] sm:aspect-[16/10] overflow-hidden bg-slate-100 dark:bg-slate-800">
        ...
        {/* Top Badges: Primary Category & Available Units (Clean single row, no wrapping) */}
        <div className="absolute top-3 left-3 right-3 flex items-center justify-between gap-2 pointer-events-none">
          <span className="px-2.5 py-1 rounded-lg text-[10px] font-bold bg-slate-900/85 text-white backdrop-blur-md border border-white/20 uppercase tracking-wider shadow-sm truncate max-w-[170px]">
            {(building.categories?.[0] || building.category || 'Commercial').replace(/-/g, ' ')}
          </span>

          <span className="px-2.5 py-1 rounded-lg text-xs font-bold bg-brand-600 text-white backdrop-blur-md shadow-md border border-brand-400/30 shrink-0">
        ...
        <div className="absolute bottom-3 left-3 right-3 space-y-0.5">
          <span className="text-[10px] sm:text-[11px] font-semibold text-brand-400 uppercase tracking-wider block truncate">
            {building.tower_details || 'Commercial Project / Tower'}
          </span>
          <h3 className="text-lg sm:text-xl font-bold text-white tracking-tight font-['Outfit'] drop-shadow-sm truncate">
            {building.name}
          </h3>
